...
次の点に注意してください。 link:/cloudinsights/cs_restrict_user_access.html["ユーザアクセスブロック"]：
...
....
注: 1つのrest-role (_arwrole_or_csrestrole_--)がすでに追加されている場合、2つ目のrest-roleを追加する必要はありません。以下の例のように、API権限を追加するだけです。
...
security login create -user-or-group-name csuser -application http -authmethod password -role arwrole
....
詳細については、 link:concept_cs_integration_with_ontap_arp.html["ONTAP によるランサムウェア対策との統合"]
