...
[role="lead"]
NetApp Kubernetes Monitoring Operatorのインストールと設定はカスタマイズできます。
...
|  | プロキシ形式:プロキシ:サーバー:ポート:ユーザー名:パスワード: noProxy:isTelegrafProxyEnabled:isAuProxyEnabled:isFluentbitProxyEnabled:isCollectorProxyEnabled: | 顧客がプロキシを設定する場合はオプションです。これは通常、顧客の法人代理店です。 


|  | agentUUID | すべてのCloud Insights オペレータのインストールで一意に識別される文字列。これは通常、Cloud Insights によって自動的に設定されます。 


| テレグラフ |  | オペレータのTelegrafインストールをカスタマイズできる設定オプション 


|  | collectionInterval | 指標の収集間隔（秒） 
