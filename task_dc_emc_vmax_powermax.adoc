...
* パフォーマンスのアクセス検証：Acquisition UnitのWebブラウザで、次のURLにアクセスします。https：//<SMI-S Hostname or IP> ：5989 / ecomconfig_where "SMI-S Hostname or IP" is the IP address or hostname of your SMI-S server.このURLは'EMC SMI-S（ECOM）サービスの管理ポータル用であり'ログイン・ポップアップが表示されます
