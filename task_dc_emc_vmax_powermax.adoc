...
* パフォーマンスのアクセス検証：Acquisition UnitのWebブラウザで、\https://<SMI-S Hostname or IP>:5989/ecomconfigに移動します。「SMI-S Hostname or IP」はSMI-SサーバのIPアドレスまたはホスト名です。このURLは'EMC SMI-S（ECOM）サービスの管理ポータル用であり'ログイン・ポップアップが表示されます
