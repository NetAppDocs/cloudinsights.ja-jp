...
* Telegraf ConfigMaps （ CM ）を編集し、古い API トークンのすべてのインスタンスを新しい API トークンに置き換えます。
+
....
...
:wq!
....
* Telegraf DemonSet （ DS ）および ReplicaSet （ RS ）を編集し、古い API トークンのすべてのインスタンスを新しい API トークンに置き換えます。
+
....
...
:wq!
....
* Kubernetes コントローラは、 Telegraf DS ポッドを自動的に再起動します。
* Telegraf RS ポッドを再起動します
+
