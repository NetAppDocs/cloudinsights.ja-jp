...
[Subscription Calculators]では、必要になると予想されるManaged Unitの数に基づいて、推定される月あたりのCloud Insightsコストが計算されます。現在の値は事前に入力されており、それらの値を調整して将来の増加予測を計画するのに役立ちます。インフラ、Kubernetes、またはその両方の値を調整できます。
...
Amazon Marketplaceのリンクをクリックすると、AWSが開きます link:https://aws.amazon.com/marketplace/pp/B07HM8QQGY["Cloud Insights の機能です"] サブスクリプションページ。サブスクリプションを完了できます。このページでは、計算ツールで入力した値が AWS のサブスクリプションページに入力されていないことに注意してください。管理対象ユニットの総数を入力する必要があります。
