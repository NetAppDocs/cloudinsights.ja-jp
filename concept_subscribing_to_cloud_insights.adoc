...
NOTE: 特に明記されていない限り、このページの情報は一般的に 環境 Cloud Insights 商用版(Basic、Standard、Premium)です。Cloud Insights の連邦版には、このページに記載されている機能の一部が含まれていない場合があります。
...
* *Federal Edition*は、米国向けのFedRAMP準拠のCloud Insights Editionを提供します政府機関。上記の商用版で利用可能な一部の機能は、Federal Editionでは利用できない場合や、機能が制限されている場合があります。これらの違いについては、可能であればCloud Insights のドキュメントに記載されています。
...
| ライブチャット^&plus;&plus;^およびテクニカルサポート | - | チェック（ &C ）； | チェック（ &C ）； 
...
| APIアクセス^&plus;^ | チェック（ &C ）； | チェック（ &C ）； | チェック（ &C ）； 


| シングルサインオン（ SSO ） | - | - | チェック（ &C ）； 


| ワークロードセキュリティユーザデータアクセス監査 | - | - | チェック（ &C ）； 


| AI/MLを使用したワークロードセキュリティ内部の脅威検出 | - | - | チェック（ &C ）； 


| ビジネスインテリジェンスおよびレポート作成&AST;&ast; | - | - | チェック（ &C ）； 
|===
&ast;一度に5つのアクティブなカスタムモニタに制限&plus; APIアクセスは、エディション&ast;&ast; 500管理ユニット以上の環境で使用可能^&plus;&plus;^ Cloud Insights 連邦版では使用できません
