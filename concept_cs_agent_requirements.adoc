...
| オペレーティングシステム | ライセンス版の次のいずれかを実行しているコンピュータ：Red Hat Enterprise Linux 7.x、8.x 64ビットCentOS 7.x 64ビットCentOS 8 Stream Ubuntu 20~22 64ビットこのコンピュータでは、他のアプリケーションレベルのソフトウェアを実行していない必要があります。専用のサーバを使用することを推奨します。 
...
| 使用可能なディスクスペース | ディスクスペースの割り当ては、次のように行います。/opt/netapp 35GB（最小） 
