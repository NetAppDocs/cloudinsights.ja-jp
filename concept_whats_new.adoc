...
。 link:/task_config_telegraf_agent_k8s.html#operator-based-install-or-script-based-install["NetApp Kubernetes Monitoring Operator"] （NKMO）はKubernetes 1.17以降を実行するすべての環境で使用でき、Cloud Insights 用のKubernetesのインストールに推奨される方法です。NKMOは、スクリプトでインストールされた監視方法よりも優れた機能です。これにより、Cloud Insights で制御可能なモニタリングの柔軟な設定が可能になり、顧客による設定介入が減少します。また、Kubernetesクラスタで実行されている他のソフトウェアを監視する機会も強化されます。
...
データ収集のアラートを受信できるようになりました link:/task_system_monitors.html#collector-monitors["イベント"]：
...
これらのモニタは、デフォルトでは_Paused _であり、アラートを送信する前に有効（再開）する必要があります。

また、Collector Warningのモニタは他のモニタタイプよりも多くのアラートを送信できるため、トラブルシューティングを行っていない限り、アラート受信者を設定しないことをお勧めします。



==== ストレージ変更ログイベントに関するアラート

Cloud Insights で生成されるストレージ変更ログイベントの監視を定義すると（ログソース_logs.cloud_insights.change_log.storage_を使用して）情報アラートが表示され、収集のたびにストレージシステムの過去の状態と新しい状態が比較されます。ONTAP ストレージでは、次のイベントに対してデフォルトモニタを使用できます。
