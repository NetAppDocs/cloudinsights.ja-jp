...
=== ワークロードセキュリティ画面の読み込みが改善されました

Cloud Insights では、オブザーバビリティとワークロードセキュリティの画面をより効率的に表示することでユーザエクスペリエンスが向上し、これらの機能間を移動する際のロード時間が短縮されました。[Workload Security]ページのURLが変更されたことに注意してください。以前のURLは自動的に新しいURLにリダイレクトされます。
...
。 link:concept_kubernetes_performance_monitoring_and_map.html["_Kubernetesのネットワークパフォーマンスとマップ_"] Kubernetesワークロード間の依存関係をマッピングすることで、トラブルシューティングを簡易化します。Kubernetesのネットワークパフォーマンスのレイテンシや異常をリアルタイムで可視化し、ユーザに影響が及ぶ前にパフォーマンスの問題を特定します。この機能は、Kubernetesのトラフィックフローを分析、監査することで全体的なコストを削減するのに役立ちます。
