...
Cloud Insightsのオブジェクトアイコンが更新され、さまざまなCloud Insightsページで使用されるアイコンの全体的なばらつきが軽減され、類似するオブジェクト（ボリューム関連オブジェクトなど）がグループ化されました。以前の「文字ベース」のアイコンは画像ベースのグラフィックに置き換えられ、Cloud Insights画面がよりユーザーフレンドリーでアクセスしやすいようになりました。

アイコンの意味がわからない場合は、次の表（ページでも使用でき link:reference_object_icons.html["オブジェクトアイコンリファレンス"] ます）では、これらすべてについて説明しています。
