...
* 次のコマンドが使用可能である必要があります：_curl、sudo、openssl、sha256sum、_and_kubectl_。最適な結果を得るには、これらのコマンドをパスに追加してください。_kubectl_は、少なくとも次のKubernetesオブジェクトにアクセスできるように設定する必要があります。_agents、clusterroles、clusterrolebindings、customresourcedefinitions、deployments、 ネームスペース、ロール、ロールバインド、シークレット、サービスアカウント、_および_services_。を参照してください link:nkmo_clusterrole_yaml.html["こちらをご覧ください"] たとえば、これらの最小クラスタロール権限を持つ.yamlファイルなどです。
...
....
社内のポリシーに従って、オペレータ用の Docker イメージをプライベート / ローカル / エンタープライズ Docker リポジトリにプッシュします。リポジトリ内のこれらのイメージへのディレクトリパスが、docker.<cluster> .cloudinsights.netapp.comのディレクトリパスと一致していることを確認します。
