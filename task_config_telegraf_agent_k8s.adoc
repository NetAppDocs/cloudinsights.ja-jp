...
* Cloud Insights がバックエンドストレージを監視していて、KubernetesをDockerコンテナランタイムとともに使用している場合、Cloud Insights はNFSおよびiSCSIのPODとPVとストレージのマッピングを表示できます。それ以外のランタイムではNFSのみが表示されます。
...
>>>>>>>>e64b5299cf9e0451417037778f9d37b84b9d9e6b * NetApp Kubernetes Monitoring Operatorインストールに使用するホストには、ターゲットのK8sクラスタと通信し、Cloud Insights 環境へのインターネット接続を確立するための_kubectl_configuredが必要です。このホストがCloud Insights にアクセスするためにプロキシを必要とする場合は、の手順に従います <<configuring-proxy-support,プロキシサポートを設定しています>> セクション。
