...
| 質問：ワークロードセキュリティがすでに設定されているSVMでこのスクリプトを実行する場合、そのスクリプトはSVMの既存のfpolicy configを使用するか、一時的に設定してプロセスを実行しますか。 


| 回答 ：ワークロードのセキュリティがすでに設定されているSVMでも、イベントレートチェッカーは正常に実行できます。影響はありません。 
