...
| エージェントがまだ SaaS に接続できない場合は、ネットアップサポートでケースをオープンしてください。Cloud Insight のシリアル番号を入力してケースをオープンし、メモしたとおりにケースをオープンしながらログを添付してください。 | ネットアップのケースをオープンする際にログを添付1. root 権限を持つ次のスクリプトを実行し、出力 zip （ cloudsecure-agent-回 します。 zip ）を共有します。A/opt/NetApp/cloudsecure/agent/bin/cloudsecure-agent-symptom-collector.sh 2 に設定します。次のコマンドを root 権限で 1 つずつ実行し、出力を共有します。AID cssys b.グループは cssys c. をCat /etc/os-release 
|===