...
* 30日が過ぎました。30日間は、エージェントが一時停止された日ではなく、前回のエージェント更新の日から開始されます。


これらのいずれの場合も、エージェントは次回のワークロードセキュリティ更新時に更新されます。エージェントの自動更新を一時停止または再開するには、_cloudsecure_config.agents API_を使用します。

image:ws_pin_agent_apis.png["WS PINエージェントAPI"]

一時停止または再開のアクションが有効になるまで、最大5分かかることがあります。現在のエージェントのバージョンは、[Workload Security]>[Collectors]ページの[Agents]タブで確認できます。

image:ws_agent_version.png["WS Agentのバージョン"]
